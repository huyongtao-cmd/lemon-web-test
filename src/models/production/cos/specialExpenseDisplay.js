...
export default {
  namespace: 'specialExpenseDisplay',
  ...
  effects: {
    *init({ payload }, { put, select }) {
      const {
        formData: { id },
        copy = false,
      } = yield select(({ specialExpenseDisplay }) => specialExpenseDisplay);
    ...
    *fetchInternalOuList({ payload }, { put, select }) {
      ...
      } = yield select(({ user, specialExpenseDisplay }) => ({
        ...user,
        ...specialExpenseDisplay,
      }));
      const output = yield outputHandle(customSelectionListByKey, {
      ...
      if (!chargeCompany) {
        ...
        yield put({
          ...
          payload: {
            ...
            chargeCompanyDesc: chargeCompany2Desc, // 费用承担公司/报销主体->用于规则校验
          },
        });
      }
    ...
    *fetchConfig({ payload }, { call, put }) {
      ...
      if (taskKey === 'COS05_01_SUBMIT_i') {
        formMode = 'EDIT';
        currentNode = 'create';
      } else if (taskKey === 'S01_06_APPLY_RES_EDIT1' || taskKey === 'S01_06_APPLY_RES_EDIT2') {
        formMode = 'EDIT';
        currentNode = 'applyEdit';
      } else if (
        taskKey === 'COS05_04_AUDITING' ||
        taskKey === 'COS05_05_FINANCE_MANGER' ||
        taskKey === 'COS05_06_FINANCE_IN_CHARGE'
      ) {
        formMode = 'EDIT';
