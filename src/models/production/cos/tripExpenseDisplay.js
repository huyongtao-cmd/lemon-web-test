  ...
  effects: {
    *init({ payload }, { put, select }) {
      const {
        formData: { id },
        copy = false,
      } = yield select(({ tripExpenseDisplay }) => tripExpenseDisplay);
      ...
      data.details = data.details.map(item => ({
        ...
        finAccSubjId2: Array.isArray(item.invoiceList)
          ? item.invoiceList.map(v => v.invoiceNo || '').join(',')
          : '',
        invIdList: Array.isArray(item.invoiceList) ? item.invoiceList.map(v => v.id) : [],
      }));
      // 当为复制时,处理id为null
      const copyObj = {};
      if (copy) {
    ...
    *fetchInternalOuList({ payload }, { put, select }) {
      ...
      } = yield select(({ user, tripExpenseDisplay }) => ({
        ...user,
        ...tripExpenseDisplay,
      }));
      const output = yield outputHandle(customSelectionListByKey, {
      ...
      if (!chargeCompany) {
        ...
        if (chargeCompanyList && chargeCompanyList.length > 0) {
          ...
          chargeCompany2Desc = chargeCompanyList[0].title;
        }
        yield put({
          type: 'updateForm',
          payload: {
            chargeCompany: chargeCompany2,
            chargeCompanyDesc: chargeCompany2Desc,
          },
        });
      }
    ...
    *fetchTripApplyList({ payload }, { put, select }) {
      const { data } = yield outputHandle(tripApplySelectPaging, { dataLimit: 'MY_NOT_USE' });
      const tripApplyList = data.rows.map(item => ({
        ...item,
        value: item.id,
        title: item.tripName,
      }));
      ...
      // if (tripApplyList.length > 0) {
      //   yield put({
      //     type: 'updateForm',
      //     payload: {
      //       accountNo: tripApplyList[0].accountNo,
      //       holderName: tripApplyList[0].holderName,
      //       bankName: tripApplyList[0].bankName,
      //       bankBranch: tripApplyList[0].bankBranch,
      //     },
      //   });
      // }
    },
    ...
    *save({ payload }, { put, select }) {
      ...
      if (id && id > 0) {
        ...
        if (formData.details != null) {
          formData.details = formData.details.map(d => {
            if (d.detailAppropriationAmt && !Array.isArray(d.detailAppropriationAmt)) {
              // eslint-disable-next-line no-param-reassign
              d.detailAppropriationAmt = Array.of(d.detailAppropriationAmt);
            }
            if (d.detailAppropriationAmt === '') {
              // eslint-disable-next-line no-param-reassign
              d.detailAppropriationAmt = Array.of('');
            }
            return d;
          });
        }
    ...
    *fetchConfig({ payload }, { call, put }) {
      ...
      if (taskKey === 'COS02_01_SUBMIT_i') {
        formMode = 'EDIT';
        currentNode = 'create';
      } else if (taskKey === 'COS02_06_APPLY_RES_EDIT1' || taskKey === 'COS02_06_APPLY_RES_EDIT2') {
        formMode = 'EDIT';
        currentNode = 'applyEdit';
      } else if (
        taskKey === 'COS02_06_AUDITING' ||
        taskKey === 'COS02_06_PROJECT_ACCOUNTING' ||
        taskKey === 'COS02_07_FINANCE_MANGER' ||
        taskKey === 'COS02_08_FINANCE_IN_CHARGE'
      ) {
        formMode = 'EDIT';
