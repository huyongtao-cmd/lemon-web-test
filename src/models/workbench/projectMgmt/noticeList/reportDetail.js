...
export default {
  namespace: 'noticeListReportDetail',
  ...
  effects: {
    ...
    *projectMemberPage({ payload }, { call, put, select }) {
      ...
      const { dataListSelected } = yield select(
        ({ noticeListReportDetail }) => noticeListReportDetail
      );
    ...
    *dailyIncrease({ payload }, { call, put, select }) {
      ...
      if (!fromQs().id) {
        const { data: datum } = yield outputHandle(
          dailyIncreaseRq,
          params,
          'noticeListReportDetail/success'
        );
        data = datum;
        ...
        if (data.id) {
          ...
          router.push(
            `/workTable/projectMgmt/noticeList/noticeListReportDetail?id=${id}&projectId=${projectId}&phaseId=${phaseId}&sourceType=REPORT&scene=DAILY_REPORT&mode=EDIT`
          );
        } else {
