...
export default {
  namespace: 'noticeListReportEdit',
  ...
  effects: {
    ...
    *projectMemberPage({ payload }, { call, put, select }) {
      ...
      const { dataListSelected } = yield select(({ noticeListEdit }) => noticeListEdit);
    ...
    *noticeResIdByDailyId({ payload }, { call, put, select }) {
      const { data } = yield outputHandle(noticeResIdByDailyIdRq, payload);
      yield put({
        type: `updateForm`,
        payload: {
          commitUser: data,
    ...
    *dailyIncrease({ payload }, { call, put, select }) {
      ...
      if (!fromQs().id) {
        ...
        if (data.id) {
          ...
          router.push(
            `/workTable/projectMgmt/noticeList/noticeListReportEdit?id=${id}&projectId=${projectId}&phaseId=${phaseId}&sourceType=REPORT&scene=DAILY_REPORT&mode=EDIT`
          );
        } else {
        ...
        if (params.commitFlag) {
          const { projectId, phaseId } = fromQs();
          closeThenGoto(
            `/workTable/projectMgmt/noticeList?refresh=${genFakeId(
              -1
