...
import { findLeadById, selectUsers, selectBus } from '@/services/user/management/leads';
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      yield put({
        ...
        payload: {
          ...
          formDataSource: { ...formData, productIds },
    ...
    *save({ payload }, { put, call, select }) {
      const { formData, page } = yield select(({ userOppsEdit }) => userOppsEdit);
      formData.productIds =
      ...
      if (formData.id) {
        ...
        if (response && response.ok) {
          if (response.datum.errorCode) {
            ...
            // );
            // router.push(
            //   `/sale/management/oppsdetail?id=${response.datum.id}&mode=update&tab=basic&page=opps`
            // );
    ...
    // 保存类别码
    *saveCategory(_, { call, select, put }) {
      const { formData } = yield select(({ userOppsEdit }) => userOppsEdit);
      const newFormData = omit(['productIds'], formData); // 删除对象中给定的 keys 对应的属性。
      const { status, response } = yield call(updateCategory, newFormData);
      if (status === 100) {
        // 主动取消请求
        return;
      }
    ...
    *getPageConfig({ payload }, { call, put, select }) {
    ...
    // 获取配置字段
    *getCatCodePageConfig({ payload }, { call, put, select }) {
      const { status, response } = yield call(businessPageDetailByNo, payload);
      if (status === 200) {
        yield put({
          type: 'updateState',
          payload: {
            catCodePageConfig: response.configInfo,
          },
        });
        return response;
      }
