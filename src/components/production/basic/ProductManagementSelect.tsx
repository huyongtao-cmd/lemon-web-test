...
// @ts-ignore
import {productManagementaPgingRq,} from '@/services/workbench/project';
 ...
 * 2.
 */
class ProductManagementSelect extends React.Component<Props, any> {
  ...
  getData = async (props?:any) => {
    ...
    const optionsList = ProductManagementSelect.optionsCache.filter((item:any) => equals(item.queryParam,queryParam));
    if(optionsList.length > 0){
      ...
      const output: OutputProps = await outputHandle(productManagementaPgingRq, {...params,limit:0});
      // resultOptions = output.data.rows.map((item: any) => ({...item,id:item.id,value: item.id, title: `${item.projectNo}-${item.projectName}`}));
      resultOptions = output.data.rows.map((item: any) => ({...item,id:item.id,value: item.id, title: `${item.productName}`}));
      // @ts-ignore
      ProductManagementSelect.optionsCache.push({"queryParam":queryParam,"options":resultOptions});
    }
...
export default ProductManagementSelect;
