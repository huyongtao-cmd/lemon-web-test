...
// @ts-ignore
import {allOuSelectPaging} from '@/services/production/common/select';
 ...
 * 2.
 */
class BaseAllOuSimpleSelect extends React.Component<Props, any> {
  ...
  getData = async () => {
    const output: OutputProps = await outputHandle(allOuSelectPaging, {innerType: "INTERNAL",limit:0});
    this.setState({options: output.data.rows.map((item: any) => ({...item,id:item.id,value: item.id, title: item.valName}))});
  };
...
export default BaseAllOuSimpleSelect;
