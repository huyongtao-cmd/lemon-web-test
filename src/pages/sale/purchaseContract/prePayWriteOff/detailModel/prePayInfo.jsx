...
class PrePayInfo extends PureComponent {
  ...
  handleDocType = (key, data) => {
    const { form, dispatch, prePayWriteOffDetail } = this.props;
    const { formData } = prePayWriteOffDetail;
  ...
  pageFieldMode = fieldMode => {
    ...
    const { fieldsConfig, formData } = prePayWriteOffDetail;
    const isEdit =
  ...
  renderInfoPageConfig = () => {
    const { prePayWriteOffDetail } = this.props;
    const { pageConfig, formData, fieldsConfig } = prePayWriteOffDetail;
    const readOnly = fieldsConfig.taskKey !== 'ACC_A83_01_SUBMIT_i';
    if (pageConfig) {
  ...
  renderRelatedPageConfig = () => {
    const { mode } = fromQs();
    const { prePayWriteOffDetail } = this.props;
    const { pageConfig, formData, opportunityList, fieldsConfig, docNoList } = prePayWriteOffDetail;
    const readOnly = fieldsConfig.taskKey !== 'ACC_A83_01_SUBMIT_i';
    if (pageConfig) {
      if (!pageConfig.pageBlockViews || pageConfig.pageBlockViews.length < 1) {
        return <div />;
      ...
      const fields = [
        ...
        mode === 'view' && formData.prePaymentNo ? (
          <Field
            name="prePaymentNo"
            key="prePaymentNo"
          ...
          >
            <Link
              className="tw-link"
              to={getLink('prePayment', null, {
                id: formData.prePaymentId,
                scene: formData.prePaymentScene,
              })}
            >
  ...
  renderApplicantFinancePageConfig = () => {
    const { prePayWriteOffDetail } = this.props;
    const { pageConfig, formData, receivingIdList, fieldsConfig } = prePayWriteOffDetail;
    const readOnly = fieldsConfig.taskKey !== 'ACC_A83_01_SUBMIT_i';
    if (pageConfig) {
      if (!pageConfig.pageBlockViews || pageConfig.pageBlockViews.length < 1) {
        return <div />;
            ...
            rules: [
              {
                ...
                message: `请选择${pageFieldJson.receivingId.displayName}`,
              },
            ],
          }}
        >
          <AsyncSelect
            ...
            placeholder={`请选择${pageFieldJson.receivingId.displayName}`}
  ...
  renderAccountInfoPageConfig = () => {
    const { prePayWriteOffDetail } = this.props;
    const { pageConfig, formData, fieldsConfig, finalPaymentIdList } = prePayWriteOffDetail;
    const { mode } = fromQs();
    if (pageConfig) {
      if (!pageConfig.pageBlockViews || pageConfig.pageBlockViews.length < 1) {
        return <div />;
  ...
  render() {
    const { loading, prePayWriteOffDetail, dispatch, form } = this.props;
    const { getFieldDecorator } = form;
    const { pageConfig, fieldsConfig } = prePayWriteOffDetail;
    const readOnly = fieldsConfig.taskKey !== 'ACC_A83_01_SUBMIT_i';
    let displayPayRecord = true;
    if (fieldsConfig.taskKey) {
      displayPayRecord = fieldsConfig.taskKey !== 'ACC_A83_01_SUBMIT_i';
    }
    return (
      <>
        ...
        {/* {displayPayRecord && (
          <>
            <Divider dashed />
            <Card className="tw-card-adjust" bordered={false}>
              <div className="tw-card-title">付款申请记录</div>
              {pageConfig.pageBlockViews &&
                pageConfig.pageBlockViews.length > 1 && (
                  <EditableDataTable
                    {...payRecordTableProps(DOMAIN, dispatch, loading, form, prePayWriteOffDetail)}
                  />
                )}
            </Card>
          </>
