...
export function payRecordTableProps(DOMAIN, dispatch, loading, form, mode, paymentApplyEdit) {
  const { payRecordList, formData, fieldsConfig, pageConfig } = paymentApplyEdit;
  const pageFieldJson = {};
  if (pageConfig) {
  ...
  const onCellChanged = (rowIndex, rowField) => rowFieldValue => {
    ...
    if (rowField === 'psubjecteCompany') {
      ...
      if (typeof newVal === 'number') {
        if (mathAdd(paymentAmtTotal, newVal) <= formData.currPaymentAmt) {
          dispatch({
            type: `${DOMAIN}/updateState`,
            payload: {
              payRecordList: update(payRecordList, {
