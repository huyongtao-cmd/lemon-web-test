  ...
  effects: {
    *init({ payload }, { put, select }) {
      const {
        formData: { id },
        copy = false,
      } = yield select(({ otherPaymentDisplay }) => otherPaymentDisplay);
    ...
    *fetchOtherPaymentType({ payload }, { put, select }) {
      const output = yield outputHandle(customSelectionListByKey, {
        key: 'CUS:OTHER_PAYMENT:TYPE',
      });
      const list = output.data.map(item => ({
        ...item,
        value: item.selectionValue,
    ...
    *fetchInternalOuList({ payload }, { put, select }) {
      ...
      } = yield select(({ user, otherPaymentDisplay }) => ({
        ...user,
        ...otherPaymentDisplay,
      }));
      const output = yield outputHandle(customSelectionListByKey, {
    ...
    *fetchCustomSetting({ payload }, { put, select }) {
     ...
     * @param put
     * @param select
     * @return {IterableIterator<Promise<OutputProps | OutputProps | any>|*>}
     */
    *fetchAbList({ payload }, { put, select }) {
      const { data } = yield outputHandle(abSelectPaging, { limit: 0, ...payload });
    ...
    *fetchConfig({ payload }, { call, put }) {
      ...
      if (taskKey === 'COS12_01_SUBMIT_i') {
        formMode = 'EDIT';
        currentNode = 'create';
      } else if (
        taskKey === 'COS12_04_AUDITING' ||
        taskKey === 'COS12_05_FINANCE_MANGER' ||
        taskKey === 'COS12_06_FINANCE_IN_CHARGE'
      ) {
        formMode = 'EDIT';
