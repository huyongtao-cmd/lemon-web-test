...
// namespace声明
const DOMAIN = 'regularExpenseDisplay';
const TOTAL_ROW_ID = 'TOTAL';
const procKey = 'COS_S01';

/**
 * 常规报销 综合展示页面
 */
@connect(({ loading, dispatch, regularExpenseDisplay, user: { user } }) => ({
...
class RegularExpenseDisplay extends React.PureComponent {
  state = {
    visible: false,
  };
  ...
  componentDidMount() {
    ...
    this.callModelEffects('updateForm', {
      ...
      expenseClaimResId: extInfo.resId,
    });
      ...
      taskId &&
        this.callModelEffects('fetchConfig', { taskId, accountingDate }).then(res => {
          this.callModelEffects('fetchFinancialAccSubjListLow').then(listRes => {
            ...
            const { taskKey } = res;
            if (taskKey === 'S01_06_AUDITING' || taskKey === 'S01_06_PROJECT_ACCOUNTING') {
  ...
  render() {
    ...
    const diabledView = taskKey === 'S01_06_AUDITING' || taskKey === 'S01_06_PROJECT_ACCOUNTING';

    // 处理申请人修改节点的按钮
    let wrappedFieldsConfig = fieldsConfig;
    if (
      fieldsConfig &&
      (fieldsConfig.taskKey === 'S01_06_AUDITING' ||
        fieldsConfig.taskKey === 'S01_06_PROJECT_ACCOUNTING')
    ) {
    ...
    // 其他流程
    const allBpm = [{ docId: formData.id, procDefKey: procKey, title: '常规报销流程' }];
        ...
        <BpmWrapper
          ...
          onBtnClick={({ operation, bpmForm }) => {
            ...
            if (
              ...
              if (
                (taskKey === 'S01_06_PROJECT_ACCOUNTING' || taskKey === 'S01_06_AUDITING') &&
                key === 'FLOW_PASS'
              ) {
        ...
        >
          ...
          >
            ...
            <FormItem
              ...
              descList={[{ value: formData.chargeProjectId, title: formData.chargeProjectDesc }]}
            />
...
export default RegularExpenseDisplay;
