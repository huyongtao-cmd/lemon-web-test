...
import { Form, Icon, Tooltip, Table } from 'antd';
// 产品化组件
...
// namespace声明
const DOMAIN = 'tripExpenseDisplay';
 ...
 * 差旅报销 综合展示页面
 */
@connect(({ loading, dispatch, tripExpenseDisplay, user: { user } }) => ({
...
class TripExpenseDisplay extends React.PureComponent {
  state = {
    visible: false,
  };
  ...
  componentDidMount() {
    ...
    const { id, copy, mode, currentNode = 'create', taskId, ...rest } = fromQs();
    ...
    this.callModelEffects('updateForm', { id, chargeBuId: extInfo.baseBuId, ...rest });
    const { chargeClassification } = fromQs();
    this.callModelEffects('init').then(data => {
      ...
      taskId &&
        this.callModelEffects('fetchConfig', { taskId, accountingDate }).then(res => {
          this.callModelEffects('fetchFinancialAccSubjListLow').then(listRes => {
            ...
            const { taskKey } = res;
            if (taskKey === 'COS02_06_AUDITING' || taskKey === 'COS02_06_PROJECT_ACCOUNTING') {
      ...
      this.callModelEffects('fetchBusinessAccItem', {
        ...
        projType:
          chargeClassification || data.chargeClassification || formData.chargeClassification,
      });
    ...
    this.callModelEffects('fetchTripApplyList');
  }
  ...
  render() {
    ...
    const { prcId, mode } = fromQs();
    const { userId, resName } = extInfo;
    const { visible, selectedInvos = [], detailsId } = this.state;
    const { taskKey } = fieldsConfig;
    const diabledView =
      taskKey === 'COS02_06_AUDITING' || taskKey === 'COS02_06_PROJECT_ACCOUNTING';

    // 处理申请人修改节点的按钮
    let wrappedFieldsConfig = fieldsConfig;
    if (
      fieldsConfig &&
      (fieldsConfig.taskKey === 'COS02_06_AUDITING' ||
        fieldsConfig.taskKey === 'COS02_06_PROJECT_ACCOUNTING')
    ) {
    ...
    // 其他流程
    const allBpm = [{ docId: formData.id, procDefKey: procKey, title: '差旅报销流程' }];
        ...
        >
          ...
          >
            ...
            <FormItem
              ...
              required
              disabled
              // disabled={currentNode !== 'create' || formData.relatedDocId}
              // onChange={(value, option) => {
              //   this.callModelEffects('updateForm', {
            ...
            <FormItem
              ...
              onChange={(value, option) => {
                if (option.length > 0) {
                  ...
                  this.updateModelState({ tripApplyList: [] });
                }
              }}
            />
            ...
            <FormItem
              ...
              fieldMode={prcId || mode === 'DESCRIPTION' ? 'LINK' : 'EDIT'} //判断是否是新增或修改页，不是就可跳转
              descList={[
                ...tripApplyList,
              ...
              onChange={(value, option) => {
                if (option.length > 0) {
                  this.callModelEffects('updateForm', {
                    ...
                    relatedBudgetIdDesc: option[0].relatedBudgetIdDesc,
                    chargeClassification: option[0].chargeClassification,
                    chargeProjectId: option[0].chargeProjectId,
                    chargeProjectIdDesc: option[0].chargeProjectName,
                    chargeBuId: option[0].chargeBuId,
                    chargeBuName: option[0].chargeBuName,
                    chargeCompany: option[0].chargeCompany,
                    chargeCompanyName: option[0].chargeCompanyName,
                    chargeCompanyDesc: option[0].chargeCompanyName,
                  });
                  this.callModelEffects('fetchBusinessAccItem', {
                    docType: formData.expenseDocType,
                    buId: option[0].chargeBuId,
                    projType: option[0].chargeClassification,
                  });
              ...
              descriptionRender={
                <Link
                  className="tw-link"
                  to={`/workTable/user/myTripApplyDisplay?id=${
                    formData.relatedDocId
                  }&mode=DESCRIPTION`}
                >
                  {formData.relatedDocIdDesc}
                </Link>
              } //跳转至出差申请详情页
            />
            ...
            <FormItem
              ...
              descList={[{ value: formData.chargeProjectId, title: formData.chargeProjectIdDesc }]}
            />
            ...
            <FormItem
              ...
              required
              disabled={
                formData.chargeProjectId || currentNode !== 'create' || formData.relatedDocId
              }
              onChange={(value, option) => {
                if (option.length > 0) {
              ...
              descList={[
                ...
                { value: formData.chargeBuId, title: formData.chargeBuIdDesc },
              ]}
            />
          ...
          {(formMode === 'DESCRIPTION' || !isNil(taskKey)) && (
...
export default TripExpenseDisplay;
