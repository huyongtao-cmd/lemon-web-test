...
class LoanExpenseDisplay extends React.PureComponent {
  ...
  render() {
    ...
    // 其他流程
    const allBpm = [{ docId: formData.id, procDefKey: procKey, title: '借款核销报销流程' }];
        ...
        >
          ...
          >
            ...
            <FormItem
              ...
              onChange={(value, option) => {
                if (option.length > 0) {
                  const chargeCompanyList = internalOuList.filter(
                    e => e.selectionKey === option[0].chargeCompany
                  );
                  const { selectionName } = chargeCompanyList[0];
                  dispatch({
                    type: `${DOMAIN}/updateFormForEditTable`,
                    payload: {
                      chargeCompanyDesc: selectionName,
                    },
                  });
                  if (!isEmpty(details)) {
                    details.forEach((key, index) => {
                      ...
                      if (!isNil(v.invoiceList) && !isEmpty(v.invoiceList)) {
                        v.invoiceList.forEach(item => {
                          ...
                          if (item.purchaserName && item.purchaserName !== selectionName) {
                            tt.push('抬头不一致');
                          }
