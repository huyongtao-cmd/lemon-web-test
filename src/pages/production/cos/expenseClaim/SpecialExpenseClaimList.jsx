...
class SpecialExpenseClaimList extends React.PureComponent {
  fetchData = async params => {
    const wrappedParam = { ...params, myFlag: true, expenseDocType: 'OTHERS' };
    ...
    const columns = [
      {
        ...
        render: (value, row) => (
          <Link
            onClick={() =>
              router.push(`/plat/othersExpense/othersExpenseApply?id=${row.id}&mode=DESCRIPTION`)
            }
          >
            {value}
          </Link>
        ...
        <SearchTable
          ...
          onEditClick={data => {
            if (data.expenseClaimStatus === 'CREATE') {
              router.push(`/plat/othersExpense/othersExpenseApply?id=${data.id}&mode=EDIT`);
            } else {
...
export default SpecialExpenseClaimList;
