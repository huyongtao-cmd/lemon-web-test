...
const DOMAIN = 'vacationFlowCreateNew';

@connect(({ loading, vacationFlowCreateNew, dispatch, user: { user } }) => ({
  loading,
...
class VacationApplyCreate extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({ type: `${DOMAIN}/fetchInternalOuList` });
  }
  ...
  onCellChanged = (value, row, keys) => rowFieldValue => {
    ...
    const {
      vacationFlowCreateNew: { detailEntityList },
      dispatch,
    } = this.props;
  ...
  onDeleteItems = () => {
    const {
      dispatch,
      form: { setFields },
      vacationFlowCreateNew: { formData, detailEntityList },
    } = this.props;
