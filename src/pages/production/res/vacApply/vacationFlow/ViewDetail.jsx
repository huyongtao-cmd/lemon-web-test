...
class ViewDetail extends PureComponent {
  componentDidMount() {
    const { dispatch, user, detailId } = this.props;
    dispatch({
    ...
    return (
      <PageHeaderWrapper>
        <Card
          ...
          style={{ marginTop: '6px' }}
          title={
            <Title icon="profile" id="ui.menu.plat.vacation.view" defaultMessage="休假申请详情" />
          }
          bordered={false}
        >
          <DescriptionList size="large" col={2}>
            <Description term="申请人">{resVacationApply.apprResName || ''}</Description>
            <Description term="公司">{resVacationApply.companyDesc || ''}</Description>
            ...
            <DescriptionList size="large">
              <Description term="人事提交附件">
                ...
                <FormItem
                  ...
                  fileList={hrattachments}
                  preview
                />
              </Description>
              <Description term="休假后补充清单">
                {resVacationApply.addFlag === '1' && <pre>是</pre>}
                {resVacationApply.addFlag === '0' && <pre>否</pre>}
              </Description>
