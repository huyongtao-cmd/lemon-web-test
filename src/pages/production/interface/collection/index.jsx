...
import {
  collectionPagingRq,
  collectionDeleteRq,
  insertCollectionToU8cRq,
} from '@/services/production/interface';

const DOMAIN = 'collectionList';
...
class PageList extends React.PureComponent {
  ...
  fetchData = async params => {
    ...
    const { response } = await collectionPagingRq(restParams);
    return response.data;
  };

  deleteData = async keys =>
    outputHandle(collectionDeleteRq, { ids: keys.join(',') }, undefined, false);

  changeStatus = async parmars => {
    const { response } = await insertCollectionToU8cRq(parmars);
    return response;
  };
        ...
        render: (value, row, index) => (
          <Link
            onClick={() =>
              ...
              router.push(`/plat/interface/collection/detail?id=${row.id}&mode=DESCRIPTION`)
            }
          >
            {value}
          </Link>
    ...
    return (
      <PageWrapper>
        ...
        <SearchTable
          ...
          onAddClick={() => router.push('/plat/interface/collection/edit')}
          onEditClick={data => {
            ...
            router.push(`/plat/interface/collection/edit?id=${data.id}&mode=EDIT`);
          }}
          deleteData={data => {
            const { selectedRows } = getInternalState();
            const tt = selectedRows.filter(v => v.syncStatus === 'SYNCED');
