...
import { paySaveRq, payPagingRq } from '@/services/production/interface';

import styles from '../style.less';

const DOMAIN = 'payList';

@connect(({ loading, dispatch, payList }) => ({
  loading,
  dispatch,
  ...payList,
}))
...
class index extends Component {
  ...
  modifyData = async params => {
    ...
    const { response } = await paySaveRq(selectedRows);
    return response;
  };
  ...
  handleCancel = e => {
    ...
    if (isNil(e)) {
      closeThenGoto(`/plat/interface/pay?refresh=${genFakeId(-1)}`);
    }
  };
