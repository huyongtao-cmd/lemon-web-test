...
// @ts-ignore
import { payPagingRq, payDeleteRq, insertPayToU8cRq } from '@/services/production/interface';

const DOMAIN = 'payList';

@connect(({ loading, dispatch, collectionList }) => ({
  loading,
  dispatch,
  ...collectionList,
}))
class PageList extends React.PureComponent {
  ...
  fetchData = async params => {
    ...
    const { response } = await payPagingRq(restParams);
    return response.data;
  };

  deleteData = async keys => outputHandle(payDeleteRq, { ids: keys.join(',') }, undefined, false);

  changeStatus = async parmars => {
    const { response } = await insertPayToU8cRq(parmars);
    return response;
  };
        ...
        render: (value, row, index) => (
          <Link
            onClick={() =>
              ...
              router.push(`/plat/interface/pay/detail?id=${row.id}&mode=DESCRIPTION`)
            }
          >
            {value}
          </Link>
    ...
    return (
      <PageWrapper>
        ...
        <SearchTable
          ...
          onAddClick={() => router.push('/plat/interface/pay/edit')}
          onEditClick={data => {
            ...
            router.push(`/plat/interface/pay/edit?id=${data.id}&mode=EDIT`);
          }}
          deleteData={data => {
            const { selectedRows } = getInternalState();
            const tt = selectedRows.filter(v => v.syncStatus === 'SYNCED');
