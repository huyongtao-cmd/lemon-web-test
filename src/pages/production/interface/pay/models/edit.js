import { paySaveRq, payDetailRq } from '@/services/production/interface';
...
export default {
  ...
  effects: {
    *queryDetails({ payload }, { call, put, select }) {
      const { data } = yield outputHandle(payDetailRq, payload);
    ...
    *interfacePayAdd({ payload }, { call, put, select }) {
      const { ...params } = payload;

      const { data } = yield outputHandle(paySaveRq, [params], 'payEdit/success');

      message({ type: 'success' });

      if (!fromQs().id) {
        router.push(`/plat/interface/pay/edit?id=${data[0].id}`);
      } else {
        yield put({
          type: 'queryDetails',
          payload: {
