...
// @ts-ignore
import {
  willCollectionPagingRq,
  willCollectionDeleteRq,
  insertWillCollectionToU8cRq,
...
class PageList extends React.PureComponent {
  ...
  fetchData = async params => {
    ...
    const { response } = await willCollectionPagingRq(restParams);
    return response.data;
  };

  deleteData = async keys =>
    outputHandle(willCollectionDeleteRq, { ids: keys.join(',') }, undefined, false);

  changeStatus = async parmars => {
    const { response } = await insertWillCollectionToU8cRq(parmars);
    return response;
  };
        ...
        render: (value, row, index) => (
          <Link
            onClick={() =>
              ...
              router.push(`/plat/interface/receivable/detail?id=${row.id}&mode=DESCRIPTION`)
            }
          >
            {value}
          </Link>
    ...
    return (
      <PageWrapper>
        ...
        <SearchTable
          ...
          onAddClick={() => router.push('/plat/interface/receivable/create')}
          onEditClick={data => {
            ...
            router.push(`/plat/interface/receivable/edit?id=${data.id}&mode=EDIT`);
          }}
          deleteData={data => {
            const { selectedRows } = getInternalState();
            const tt = selectedRows.filter(v => v.syncStatus === 'SYNCED');
