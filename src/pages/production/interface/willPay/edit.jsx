...
const DOMAIN = 'willPayEdit';

// dispatch是触犯标志，我要开始调用方法了

// 连接数据（redux）
@connect(({ user: { user }, loading, willPayEdit, dispatch }) => ({
  loading,
  ...willPayEdit,
  dispatch,
  user,
...
// react基本页面结构
class willPayListEdit extends Component {
  // 状态、数据
  state = {};
  ...
  handleSave = () => {
    ...
    validateFieldsAndScroll((error, values) => {
      ...
      if (!error) {
        dispatch({
          type: `${DOMAIN}/interfaceWillPayAdd`,
          payload: {
            ...newFormData,
            ...restValues,
            delIds,
  ...
  render() {
    const { dispatch, formData, form, loading, mode } = this.props;
    const { detailEntities = [], delIds = [] } = formData;
    const { scene } = this.state;
    const disabled = mode === 'DESCRIPTION';
    const disabledBtn =
...
export default willPayListEdit;
