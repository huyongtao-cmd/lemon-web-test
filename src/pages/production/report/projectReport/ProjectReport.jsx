...
// @ts-ignore
import { projectReportPaging } from '@/services/production/report';
...
@connect(({ loading, dispatch, projectReport, user: { user } }) => ({
  // treeLoading: loading.effects[`${DOMAIN}/init`],
  dispatch,
  ...projectReport,
  user,
}))
class ProjectReport extends React.PureComponent {
  state = {};
  ...
  fetchData = async params => {
    ...
    const { data } = await outputHandle(projectReportPaging, wrappedParam);
  ...
  renderSearchForm = extInfo => [
    <SearchFormItem
      key="projectId"
      fieldType="ProjectManagementSelect"
      label="项目"
      fieldKey="projectId"
      defaultShow
      advanced
      queryParam={{
        projectStatusList: ['ACTIVE', 'CLOSE'].join(','),
      }}
    />,
