...
const DOMAIN = 'buResInfo';

@connect(({ dispatch, loading, buResInfo }) => ({
  dispatch,
  buResInfo,
  loading,
}))
class BuResInfo extends PureComponent {
  ...
  render() {
    const { loading, buResInfo, dispatch, buId } = this.props;
    const { listTableData = [], roleTableData = [], searchForm, pageConfig } = buResInfo;
    ...
    const tableProps = {
      ...
      searchBarForm: [
        ...
        {
          title: '当量系数范围从',
          dataIndex: 'eqvaRatioStart',
          options: {
            initialValue: searchForm.eqvaRatioStart,
          },
          tag: <InputNumber className="x-fill-100" placeholder="请输入当量系数范围从" />,
        },
        {
          title: '当量系数范围止',
          dataIndex: 'eqvaRatioEnd',
          options: {
            initialValue: searchForm.eqvaRatioEnd,
          },
          tag: <InputNumber className="x-fill-100" placeholder="请输入当量系数范围止" />,
        },
      ].filter(Boolean),
      columns: [
        pageFieldJson.resId.visibleFlag && {
        ...
        pageFieldJson.buRoleName.visibleFlag && {
          title: `${pageFieldJson.buRoleName.displayName}`,
          sortNo: `${pageFieldJson.buRoleName.sortNo}`,
          dataIndex: 'roleName',
          width: 150,
        },
        ...
        pageFieldJson.eqvaRatio.visibleFlag && {
          title: `${pageFieldJson.eqvaRatio.displayName}`,
          sortNo: `${pageFieldJson.eqvaRatio.sortNo}`,
          dataIndex: 'eqvaRatio',
          align: 'center',
          width: 100,
        },
        ...
        pageFieldJson.salaryMethod.visibleFlag && {
          title: `${pageFieldJson.salaryPeriod.displayName}`,
          sortNo: `${pageFieldJson.salaryPeriod.sortNo}`,
          dataIndex: 'salaryPeriodDesc',
          align: 'center',
          width: 100,
        },
        pageFieldJson.salaryMethod.visibleFlag && {
          title: `${pageFieldJson.salaryMethod.displayName}`,
          sortNo: `${pageFieldJson.salaryMethod.sortNo}`,
        ...
        pageFieldJson.eqvaRatioDetail.visibleFlag && {
          title: pageFieldJson.eqvaRatioDetail.displayName || '详情',
          align: 'center',
          render: (value, row, key) => (
            <Link
              className="tw-link"
              to={`/org/bu/eqvaRatio/index?buId=${buId}&resId=${
                row.resId
              }&readOnly=view&from=/org/bu/particulars`}
            >
              {pageFieldJson.eqvaRatioDetail.REMARK || '当量系数/额定当量调整记录'}
            </Link>
          ),
        },
      ]
