...
const DOMAIN = 'planListEdit';

@connect(({ loading, planListEdit, dispatch }) => ({
  loading,
  ...planListEdit,
  dispatch,
}))
...
class index extends Component {
      ...
      if (!error) {
        dispatch({
          ...
          if (from === 'PLAN') {
            res &&
              closeThenGoto(
                `/workTable/projectMgmt/projectPlanMgmt?refresh=${genFakeId(
                  -1
                )}&projectId=${projectId}`
              );
          } else {
            res &&
              closeThenGoto(
                `/workTable/projectMgmt/sessionMgmt?refresh=${genFakeId(-1)}&projectId=${projectId}`
              );
          }
        });
      }
  ...
  fetchTree = async () => {
    const { projectId } = fromQs();
    const { data } = await outputHandle(projectPlanListRq, { projectId, limit: 0 });
    return data.rows.map(item => ({ ...item, title: item.planNo }));
  };
