...
class indexCom extends Component {
  ...
  componentDidMount() {
    ...
    if (id) {
      ...
      dispatch({
        type: `${DOMAIN}/noticeResIdByDailyId`,
        payload: { id },
      });
  ...
  getProjectMember = parmas => {
    ...
    dispatch({
      ...
      const { projectAllMemberList } = this.props;
      const list = projectAllMemberList
        .filter(item => {
          if (item.memberType === 'INTERNAL_RES' || item.memberType === 'EXTERNAL_RES') {
            return { ...item, projectMemberId: item.id };
          }
          return false;
        })
        .map(v => v.memberResId);
  ...
  // 行编辑触发事件
  onRelatedPlanCellChanged = (rowId, value, name) => {
    const { relatedPlanList, dispatch } = this.props;

    const index = relatedPlanList.findIndex(v => v.id === rowId);
      ...
      },
      {
        ...
        render: (val, row, index) => (
          <Selection
            ...
            allowClear={false}
            onChange={e => {
              this.onRelatedPlanCellChanged(row.id, e, 'executeStatus');
              if (e === 'FINISHED') {
                this.onRelatedPlanCellChanged(row.id, 100, 'progress');
              }
            }}
          />
        ),
      },
      {
        ...
        render: (val, row, index) => (
          <InputNumber
            ...
            max={100}
            onChange={e => {
              this.onRelatedPlanCellChanged(row.id, e || 0, 'progress');
            }}
          />
        ),
      },
      {
        ...
        render: (val, row, index) => (
          <Input
            value={val}
            onChange={e => {
              this.onRelatedPlanCellChanged(row.id, e.target.value, 'remark');
            }}
          />
        ),
      },
  ...
  handleModelOk = (e, selectedRows) => {
    const { dispatch, relatedPlanList = [] } = this.props;
    const { sourceType } = this.state;

    dispatch({
      type: `${DOMAIN}/updateState`,
      payload: {
        relatedPlanList: relatedPlanList.concat(
          selectedRows
            .filter(v => !relatedPlanList.map(item => item.projectPlanId).includes(v.id))
            .map((v, i) => ({
              ...v,
              projectPlanId: v.id,
              sortNo: i + 1,
              sourceType,
              roleName: v.memberRoleAndName,
            }))
            .map((v, i) => ({
              ...v,
              sortNo: i + 1,
  ...
  render() {
    ...
    return (
      <PageWrapper>
        ...
        >
          <Selection.Columns
            value={formData.commitUser ? formData.commitUser : []} //没有数据就为空
            className="x-fill-100"
            source={userList}
